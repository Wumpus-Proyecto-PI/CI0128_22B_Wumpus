...
namespace PI.Controllers
{
    ...
    public class AnalisisController : Controller
    {
        ...
        public static bool ExisteMetaDeVentas(DateTime fechaAnalisis)
        {
            bool resultado = false;
            ProductoHandler productoHandler = new ProductoHandler();
        ...
        private static int DeterminarPasoDisponibles(AnalisisModel analisis)
        {
            int pasoDisponible = 1;

            List<bool> pasosActivos = new();
            pasosActivos.Add(HayPuestos(analisis));
            pasosActivos.Add(ContieneGastosFijos(analisis));
            pasosActivos.Add(ContieneGastosVariables(analisis));
            pasosActivos.Add(ExisteMetaDeVentas(analisis.FechaCreacion));

            int indexPrimerDesactivado = pasosActivos.FindIndex(x => x == false);
            pasoDisponible = indexPrimerDesactivado < 0? 6 : indexPrimerDesactivado + 1;
            return pasoDisponible; 
        }
    }
}
